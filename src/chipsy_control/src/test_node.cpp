...
void sub_callback(const sensor_msgs::JointState::ConstPtr &jointstates)
{
  std_msgs::Float64 msg;
  std::cout<<atan((jointstates->position[0]-2.5)/0.5)<<"\n";
  msg.data=atan((jointstates->position[0]-2.5)/0.5);
  pub.publish(msg);
}
